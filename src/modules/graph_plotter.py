...
class GraphPlotterModule(BaseModule):
    ...
    async def _plot_2d(
        ...
        try:
            ...
            plt.plot(x, y, linewidth=2)  # Parametre yok!
            plt.grid(True, alpha=0.3)
            plt.xlabel("x") # Tanımlı değil!
            plt.ylabel("y")
            plt.title(f'f(x) = {expression}')
            # Metod yok!
            
            png_path = self.cache_dir / f"{hash(expression)}.png" # Tanımlı değil!
            plt.savefig(png_path, dpi=150, bbox_inches="tight") # Metod yok!
